...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        let backgroundImage = SKSpriteNode(imageNamed: "backgroundSky1")
        ...
        _ = Timer.scheduledTimer(withTimeInterval: 6.0, repeats: true) { timer2 in
            self.create1()
        }
        _ = Timer.scheduledTimer(withTimeInterval: 7.0, repeats: true) { timer3 in
            self.create2()
        }
        _ = Timer.scheduledTimer(withTimeInterval: 8.0, repeats: true) { time4 in
            self.create3()
        }
    }
    ...
    func create1(){
        let create1 = SKSpriteNode(imageNamed: "objectBird3")
        create1.setScale(1)
        create1.zPosition = 4
        ...
        create1.position = CGPoint(x: 844, y: Int.random(in: 50..<360))
    ...
    func create2(){
        let create2 = SKSpriteNode(imageNamed: "objectCloud1")
        create2.setScale(1)
        create2.zPosition = 4
        ...
        create2.position = CGPoint(x: 844, y: Int.random(in: 50..<360))
        
        let path = UIBezierPath()
        path.move(to: .zero)
        path.addLine(to: CGPoint(x: -1000, y: 0))
        let movement = SKAction.follow(path.cgPath, asOffset: true, orientToPath: true, speed: 80)
        let sequence = SKAction.sequence([movement, .removeFromParent()])
        addChild(create2)
        create2.run (sequence)
    }
    func create3(){
        let create3 = SKSpriteNode(imageNamed: "objectCloud1")
        ...
        create3.position = CGPoint(x: 844, y: Int.random(in: 50..<360))
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        /*
        let blackHeart = SKSpriteNode(imageNamed: "heart1")
        blackHeart.position = CGPoint(x: self.frame.midX - 300, y: self.frame.midY + 170)
        blackHeart.setScale(1.5)
        blackHeart.zPosition = 4
        
        playerHeartsChanged(){
            if playerHearts {
                addChild(blackHeart)
                let f0 = SKTexture.init(imageNamed: "heart2.png")
                let frames: [SKTexture] = [f0]
                let normal = SKAction.animate(with: frames, timePerFrame: 0.5)
                let redo = SKAction.repeat(normal, count: 4)
                blackHeart.run(redo)
            }}
        */
    }
